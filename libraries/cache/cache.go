...
type Cache interface {
	...
	// MGet is a batch version of Get.
	...
	HSet(key string, field string, val interface{}) error
	...
	//=======list
	LPush(key string, val ...interface{}) int
	RPush(key string, val ...interface{}) int
	LPop(key string) interface{}
	RPop(key string) interface{}
	LLen(key string) int
	LRange(key string, start int, stop int) []interface{}
	LRem(key string, count int, value interface{}) int
	LSet(key string, index int, value interface{}) bool
	LIndex(key string, index int) interface{}
	LTrim(key string, start int, stop int) bool
	...
	SetExpire(key string, timeout time.Duration) error
